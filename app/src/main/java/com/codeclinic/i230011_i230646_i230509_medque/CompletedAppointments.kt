...
class CompletedAppointments : AppCompatActivity() {
    ...
    private fun loadCompletedAppointments() {
        ...
        val url = "$BASE_URL/get_completed_appointments.php"
        val jsonObject = JSONObject().apply {
            put("user_id", userId)
        }

        Log.d("CompletedAppointments", "Requesting: $url with user_id: $userId")

        val jsonObjectRequest = JsonObjectRequest(
            Request.Method.POST, url, jsonObject,
            { response ->
                Log.d("CompletedAppointments", "Response: $response")
                try {
                    ...
                    Log.e("CompletedAppointments", "Error parsing: ${e.message}")
                    Toast.makeText(this, "Error loading appointments", Toast.LENGTH_SHORT).show()
                }
            },
            { error ->
                Log.e("CompletedAppointments", "Network error: ${error.message}")
    ...
    private fun createAppointmentCard(appointment: Appointment): CardView {
        ...
        val cardView = inflater.inflate(R.layout.item_completed_appointment, appointmentsContainer, false) as CardView
        ...
        val rebookBtn = cardView.findViewById<TextView>(R.id.btnRebook)
        ...
        // Rebook button click
        rebookBtn.setOnClickListener {
            Toast.makeText(this, "Rebook feature coming soon", Toast.LENGTH_SHORT).show()
        }
